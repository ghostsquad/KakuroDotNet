namespace Kakuro.Core
{
    ...
    public class KakuroCombinationCollection : IMetaCollection<short[]>
    {
        ...
        public KakuroCombinationCollection(short sum, short spaces, IEnumerable<short> exclusions)
        {
            if (sum < 3)
            {
            ...
            if (exclusions != null)
            {
                short[] myExclusions = exclusions.Where(i => i > 0 && i < 10).Distinct().ToArray();

                foreach (var exclusion in myExclusions)
                {
